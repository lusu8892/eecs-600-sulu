// joints_trajectory_action_server: complementary server for trajectory messages
...
#include <joints_trajectory/JointsTrajActionAction.h>
...
private:
    ...
    ros::Publisher joint1_cmd_publisher_; // need a publisher to talk to the joint controller
    ros::Publisher joint2_cmd_publisher_;
    // this class will own a "SimpleActionServer" called "as_".
    actionlib::SimpleActionServer<joints_trajectory::JointsTrajActionAction> as_;
    
    // here are some message types to communicate with our client(s)
    joints_trajectory::JointsTrajActionGoal goal_; // goal message, received from client
    joints_trajectory::JointsTrajActionResult result_; // put results here, to be sent back to the client when done w/ goal
    joints_trajectory::JointsTrajActionFeedback feedback_; // not used in this example; 
    // would need to use: as_.publishFeedback(feedback_); to send incremental feedback to the client
    
    void  send_joint_commands_(vector<double> q_cmd_jnts, int njoints); // helper function to encapsulate details of how to talk to the controller; 
...
public:
    ...
    // uses goal message defined in "joints_trajectory" package
    void executeCB(const actionlib::SimpleActionServer<joints_trajectory::JointsTrajActionAction>::GoalConstPtr& goal);
};
   ...
   as_(nh_, "example_traj_action_server", boost::bind(&TrajectoryActionServer::executeCB, this, _1),false) 
{
    ...
    joint1_cmd_publisher_ = nh_.advertise<std_msgs::Float64>("joint1_pos_cmd", 1, true);
    joint2_cmd_publisher_ = nh_.advertise<std_msgs::Float64>("joint2_pos_cmd", 1, true);
...
void  TrajectoryActionServer::send_joint_commands_(vector<double> q_cmd_jnts, int njoints) { 
    ...
    std::vector<std_msgs::Float64> q_cmd_msg;
    q_cmd_msg.resize(njoints); // need to adjust to numbers of joints
    q_cmd_msg.clear();
    q_cmd_msg[0].data = q_cmd_jnts[0]; // boring...only one component; really should check size, to make sure at least this component exists
    q_cmd_msg[1].data = q_cmd_jnts[1];

    joint1_cmd_publisher_.publish(q_cmd_msg[0]); // this is how we talk to the minimal_joint_controller; change this for your target controller
    joint2_cmd_publisher_.publish(q_cmd_msg[1]);
    ROS_INFO("commanding: %f, %f", q_cmd_jnts[0], q_cmd_jnts[1]);
}
   
   
//here is where we do the work to act on goal requests
void TrajectoryActionServer::executeCB(const actionlib::SimpleActionServer<joints_trajectory::JointsTrajActionAction>::GoalConstPtr& goal) {
    ...
    while (t_stream<t_final) {
        ...
        ROS_INFO("q1_prev, q1_cmd, q1_next: %f, %f, %f",q_prev_jnts[0],q_cmd_jnts[0],q_next_jnts[0]);
        ROS_INFO("q2_prev, q2_cmd, q2_next: %f, %f, %f",q_prev_jnts[1],q_cmd_jnts[1],q_next_jnts[1]);

        //command the joints:  this implementation will depend on the specific interface to the robot;
        send_joint_commands_(q_cmd_jnts, njoints); // use helper function to encapsulate details of how to talk to the controller; 
    ...
    send_joint_commands_(q_cmd_jnts, njoints);
...
int main(int argc, char** argv) {
    ros::init(argc, argv, "joints_trajectory_action_node"); // name this node 
