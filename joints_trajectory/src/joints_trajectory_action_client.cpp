// joints_trajectory_action_client: 
// see complementary server, "joints_trajectory_action_server"
...
void doneCb(const actionlib::SimpleClientGoalState& state,
        const joints_trajectory::JointsTrajActionResultConstPtr& result) {
    ROS_INFO(" doneCb: server responded with state [%s]", state.toString().c_str());
}

int main(int argc, char** argv) {
        ros::init(argc, argv, "demo_trajectory_client_node"); // name this node 
        joints_trajectory::JointsTrajActionGoal goal; //instantiate a goal message compatible with our server, as defined in this package
        // we will command a limited-duration sinusoidal motion; define amplitude, frequency and duration
	// double omega_1 = 1.0; //rad/sec
 //    double amp_1 = 0.5; //radians
 //    double omega_2 = 2.0;
 //    double amp_2 = 1.0; 
    double omega = 1.0;
    double amp = 0.5;
	...
	double dt = 0.1; 
        ...
        actionlib::SimpleActionClient<joints_trajectory::JointsTrajActionAction> action_client("example_traj_action_server", true);
	...
	for (phase=0.0;phase<final_phase;phase+=omega*dt) {
        ...
        trajectory_point.positions[1] = -q_des; // do this for every joint, from 0 through njnts-1
        trajectory_point.velocities[1] = qdot_des;
