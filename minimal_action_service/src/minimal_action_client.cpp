...
{
public:
    MinimalActionClientClass(ros::NodeHandle* nodehandle); //"main" will need to instantiate a ROS nodehandle, then pass it to the constructor
...
private:
    ...
    actionlib::SimpleActionClient<minimal_action_service::demoAction> action_client_;
    ...
    void doneActionCB(const const actionlib::SimpleClientGoalState& state, const minimal_action_service::minimal_action_msgResultConstPtr& result);
...
MinimalActionClientClass::MinimalActionClientClass() : action_client_("minimal_action", ture) 
{ // constructor
...
MinimalActionClientClass::actionClient()
{
    ...
    while(true) {
    	...
    	if (answer.compare("Y") == 0 || answer.compare("y") == 0) {
			...
			action_client_.sendGoal(goal_,&doneActionCB); // we could also name additional callback functions here, if desired
			// action_client_.sendGoal(goal, &doneCb, &activeCb, &feedbackCb); //e.g., like this

			bool finished_before_timeout = action_client_.waitForResult(ros::Duration(5.0));
        ...
        const minimal_action_service::minimal_action_msgResultConstPtr& result) 
{
    ...
    ROS_INFO("amplitude = %d; frequency = %d; cycles = %d, outcome = [%s]", 
    		result->amplitde_output, result->frequency_output, result->cycles_output, result->outcome);
}
